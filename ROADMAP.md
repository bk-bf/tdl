...
- [x] **T-014**: Fix BUG-009 — opencode file edits not visible in nvim until focus switch; real fix: `vim.uv.new_fs_event` watcher on each open buffer's parent directory (`sync.watch_buf()`), fires `sync.sync()` on any filesystem change so nvim reloads immediately without any pane switch; `pane-focus-in` hook retained as belt-and-suspenders for unfocused buffers not yet open (see [bugs/BUG-009.md](bugs/BUG-009.md))
